...
router.post('/checkin', authenticate, async (req, res) => {
  try {
    ...
    // SECURED: Use a transaction with a row-level lock to prevent concurrent race conditions
    const newToken = await prisma.$transaction(async (tx) => {
      // 1. Lock the specific doctor row to serialize token creation
      await tx.$executeRaw`SELECT 1 FROM "Doctor" WHERE id = ${doctorId} FOR UPDATE`;

      // 2. Safely fetch current maximum token number
